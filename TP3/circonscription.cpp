...
Circonscription::Circonscription(int id, int size_max, int dist_max)
{
    votes = 0;
    this->id = id;
    max_dist = dist_max;
...
bool Circonscription::addMunicipalite(Municipalite &mun)
{
    ...
    {
        if (distance(mun, municipalite) > max_dist)
        {
...
void Circonscription::removeMunicipalite(Municipalite &target)
{
    votes -= target.votes;
    if (target.i_circ == id)
    { // if we own it, set circ to unassigned. otherwise, addmunicipality already updated its assignment
        target.i_circ = -1;
    }
...
Municipalite Circonscription::stealNeighbor(std::vector<Circonscription> &circs, std::vector<Municipalite> &munis, int x_size, int y_size)
{
    // cout << endl;
    ...
    while (!foundNeighbor)
    {
        ...
        if (newX >= 0 && newX < x_size && newY >= 0 && newY < y_size)
        {
            ...
            if (previous_circ != this->id)
            {
                ...
                bool success = this->addMunicipalite(munis[x_size * newY + newX]);
                if (success)
                {
                    foundNeighbor = true;
                    if (previous_circ != -1)
...
bool Circonscription::stealNeighborFromCirc(std::vector<Circonscription> &circs, std::vector<Municipalite> &munis, int x_size, int y_size, int circ)
{
    // cout << endl;
    // for(int i = 0; i<circs.size(); i++)
    // {
